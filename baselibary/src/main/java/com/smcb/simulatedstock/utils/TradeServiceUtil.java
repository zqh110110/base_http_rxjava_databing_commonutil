...
public class TradeServiceUtil {
    ...
    public static TradeServiceUtil instance;
    ...
    private TradeServiceUtil(){}

    public void sendMessage(Handler msgToClinet,Message msg) {
        msg.setTarget(msgToClinet);
        mMessenger.handleMessage(msgToClinet,msg);
    }

    class HandlerMsg {
        public  void handleMessage(Handler client,Message msgfromClient){};
    }


    private HandlerMsg mMessenger = new HandlerMsg()
    {
        @Override
        public void handleMessage(Handler client,Message msgfromClient)
        {
            Message msgToClient = Message.obtain(msgfromClient);//返回给客户端的消息
            msgToClient.getTarget();
    ...
    public static TradeServiceUtil getInstance() {
        if (instance == null) {
            instance = new TradeServiceUtil();
        }
        return instance;
    }
    ...
    public void start(boolean flg) {
        ...
        initSocket();
        KLog.d("tradeservice start !!");
    }

    public static void setTradeIpAndPort(String ip,int port) {
        Constant.SERVER_IP = ip;
        Constant.SERVER_PORT = port;
    }

    public static void setHQIpAndPort(String ip,int port) {
        Constant.HQ_SERVER_IP = ip;
        Constant.HQ_SERVER_PORT = port;
    }

    public static void setDELAY_TIME(int delayTime) {
        Constant.DELAY_TIME = delayTime;
    }
    ...
    private void setAccoutPwd() {
        userName = SPUtils.getInstance(TradeMainUtil.getInstance().getContext()).getStringValue(TRADE_ACCOUNT_NAME);
        userPwd = SPUtils.getInstance(TradeMainUtil.getInstance().getContext()).getStringValue(TradeServiceUtil.TRADE_POSSWORD_PWD);
    ...
    public void stop() {
        ...
        //清除账号密码
        SPUtils.getInstance(TradeMainUtil.getInstance().getContext()).setValue(TradeServiceUtil.TRADE_ACCOUNT_NAME,"");
        SPUtils.getInstance(TradeMainUtil.getInstance().getContext()).setValue(TradeServiceUtil.TRADE_POSSWORD_PWD,"");
        SocketUtil.getInstance().stopSocket();
        TradeSocUtil.getInstance().stopSocket();
        KLog.d("tradeservice stop !!");
    }
    ...
    private void initHQ() {
        ...
        SocketUtil.getInstance().runSocket(serverList, new com.sc.utils.SocketReqCallBack() {
            ...
            public void onSuccess(String info) {
                SocUtils.getInstance().reqTab(TradeMainUtil.getInstance().getContext());
            }
        });
    }
    ...
    private void initPubKey() {
        String publicKey = EncryptUtil.readKeyFile(TradeMainUtil.getInstance().getContext(), ReqConst.TRADE_PUBLIC_KEY);
        if (TextUtils.isEmpty(publicKey)) {
                ...
                public void run() {
                    TradeSocUtil.getInstance().reqCert(new SocketReqCallBack() {
                        ...
                        public void onSuccess(String resultstr) {
                            ...
                            EncryptUtil.writeKey(certstr.getBytes(), TradeMainUtil.getInstance().getContext(), ReqConst.TRADE_PUBLIC_KEY);
                            initPubKey();
                        }
                    });
                }
            ...
            try {
                PublicKey pKey = RSAUtils.getPublicKey(TradeMainUtil.getInstance().getContext().openFileInput(ReqConst.TRADE_PUBLIC_KEY));
                userLogin(null);
                ...
                TradeSocUtil.getInstance().reqData(ReqConst.CLIENT_VALIDATE, jsonStr, new SocketReqCallBack() {
                    ...
                    public void onSuccess(final String info) {
                        try {
                            ...
                            if (user != null) {
                                clientInfo = user;
                                DataCacheUtils.getAppConfig(TradeMainUtil.getInstance().getContext()).saveObject(clientInfo,CLIENTINFO_CACHE);
                            }
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    private ClientReq buildREQ() {
        ...
        req.setMAC(getLocalMac(TradeMainUtil.getInstance().getContext()));
    ...
    private void sendInfo(Message msgToClient,Object info) {
                ...
                case M_LOGIN:
                    if (clientInfo == null) {
                        if (((String) info).contains("-35")) {
                            clientInfo = (ClientInfo) DataCacheUtils.getAppConfig(TradeMainUtil.getInstance().getContext()).readObject(CLIENTINFO_CACHE);
        ...
        try {
            ...
            Handler target = msgToClient.getTarget();
            target.sendMessage(msgToClient);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
