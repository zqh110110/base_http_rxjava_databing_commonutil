...
public class AutoUtils
{
    ...
    public static void autoTextSize(View view) {
        int pxVal = 0;
        Boolean hasAutoTxSize = (Boolean) view.getTag(android.R.attr.text);

        if (view instanceof TextView && hasAutoTxSize!=null) {
            pxVal = (int) ((TextView)view).getTextSize();
            int val = getPercentWidthSize(pxVal);
            val = Math.max(val, 1);//for very thin divider
            ((TextView) view).setIncludeFontPadding(false);
            ((TextView) view).setTextSize(TypedValue.COMPLEX_UNIT_PX, val);
            view.setTag(android.R.attr.text,true);
        }
